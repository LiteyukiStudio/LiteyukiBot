...
def generate_plugin_image(event) -> Canvas:
    ...
    help_canvas.content.tips.loaded_plugin.text = Text(
        uv_size=(1, 1), box_size=(10, 0.5), parent_point=(0.5, 0.5), point=(0.5, 0.5), color=(255, 255, 255, 255), text="已加载的插件数：%s" % plugin_count, font=normal_font,
    )
    ...
    for i, plugin in enumerate(loaded_plugins_sorted):
        ...
        plugin_bg.plugin_name = Text(
            uv_size=(1, 1), box_size=(0.8, 0.5), parent_point=(0.05, 0.5), point=(0, 0.5), text=plugin_show_name, font=normal_font, color=color, dp=1
        )
