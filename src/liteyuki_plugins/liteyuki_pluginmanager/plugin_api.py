...
def generate_plugin_image(event) -> Canvas:
    ...
    help_canvas.content.head.tips = Text(
        uv_size=(1, 1), box_size=(0.5, 0.2), parent_point=(0.5, 0.7), point=(0.5, 0.5), font=normal_font, color=(255, 255, 255, 255),
        text=[
            TextSegment("绿色:轻雪插件    ", color=Color.GREEN), TextSegment("白色:第三方插件")
        ]
    )
    help_canvas.content.head.liteyuki_sign = Text(uv_size=(1, 1), box_size=(1, 0.1), parent_point=(0.5, 0.95), point=(0.5, 1), text="Generated by LiteyukiBot4.0", font=heavy_font,
                                                  color=(80, 80, 80, 255))
    ...
    for i, plugin in enumerate(loaded_plugins_sorted):
        ...
        plugin_bg.plugin_name = Text(
            uv_size=(1, 1), box_size=(0.8, 0.5), parent_point=(0.05, 0.5), point=(0, 0.5), text=plugin_show_name, font=normal_font, color=color, dp=1, force_size=True,
