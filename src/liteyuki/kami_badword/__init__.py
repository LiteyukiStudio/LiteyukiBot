...
from extraApi.rule import pluginEnable, BOT_GT_USER
...
async def badwordWarn(bot: Bot, event: Union[GroupMessageEvent, PrivateMessageEvent], state: T_State):
    if await IS_BADWORD(bot, event, state):
        ...
        3.撤回且禁言移除（默认9次）
        """
        if type(event) is GroupMessageEvent and await BOT_GT_USER(bot, event, state) and await pluginEnable(
                "kami.badword")(bot, event, state):
...
editBadword = on_command(cmd="添加违禁词", aliases={"删除违禁词", "添加全局违禁词", "删除全局违禁词"},
                         permission=SUPERUSER | GROUP_OWNER | GROUP_ADMIN, rule=pluginEnable("kami.badword"), priority=10, block=True)
listBadword = on_command(cmd="列出违禁词", rule=pluginEnable("kami.badword"),
                         permission=SUPERUSER | GROUP_OWNER | GROUP_ADMIN, priority=10, block=True)
set_time = on_command(cmd="设置禁言次数", aliases={"设置移出次数", "设置违禁词模式"}, rule=pluginEnable("kami.badword"), permission=SUPERUSER | GROUP_OWNER | GROUP_ADMIN, priority=10, block=True)
