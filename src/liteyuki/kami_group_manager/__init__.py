...
from extraApi.rule import pluginEnable, BOT_IS_OWNER, BOT_IS_ADMIN
...
ban = on_command(cmd="ban", aliases={"禁言", "解禁"}, rule=pluginEnable("kami.group_manager") & BOT_IS_ADMIN,
                 permission=SUPERUSER | GROUP_OWNER | GROUP_ADMIN, block=True)
kick = on_command(cmd="kick", aliases={"移出", "移除"}, rule=pluginEnable("kami.group_manager") & BOT_IS_ADMIN,
                  permission=SUPERUSER | GROUP_OWNER | GROUP_ADMIN, block=True)
title = on_command(cmd="头衔", rule=pluginEnable("kami.group_manager") & BOT_IS_OWNER, block=True)
