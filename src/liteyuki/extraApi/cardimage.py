...
class Cardimage:
    ...
    def __init__(self, baseImg: Image.Image):
        ...
        self.fp = os.path.join(ExConfig.cachePath, fileName)
        self.baseImg = baseImg

    @run_sync
    def addText(self, uvSize: Tuple[int | float, int | float], boxSize: Tuple[int | float, int | float],
                xy: Tuple[int | float, int | float],
                baseAnchor: Tuple[float, float],
                textAnchor: Tuple[float, float], content: str, font: str = os.path.join(ExConfig.resPath, "fonts/MiSans-Medium.ttf"),
                color: Tuple[int, int, int, int] = (0, 0, 0, 255), sizePercent: float = 1, force_size=False):
        """
        ...
        # 基类锚点绝对位置int
        baseAnchorXY = int(self.baseImg.size[0] * (baseAnchor[0] + xy[0] / uvSize[0])), int(
            self.baseImg.size[1] * (baseAnchor[1] + xy[1] / uvSize[1]))
    ...
    @run_sync
    def addImage(self, uvSize: Tuple[int | float, int | float], boxSize: Tuple[int | float, int | float],
                 xy: Tuple[int | float, int | float],
                 baseAnchor: Tuple[float, float],
