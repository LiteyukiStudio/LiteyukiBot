...
async def covid_data_format_to_message(data: dict):
    ...
    card = Cardimage(Image.open(os.path.join(ExConfig.resPath, "textures/covid19/bg.png")))
    await card.addImage(uvSize=(1, 1), boxSize=(0.2, 0.2), xy=(0, 0), baseAnchor=(0.95, 0.05), imgAnchor=(1, 0),
                        img=Image.open(os.path.join(ExConfig.resPath, "textures/covid19/新冠疫情.png")))
    await card.addText(uvSize=(1, 1), boxSize=(0.7, 0.15), xy=(0, 0), baseAnchor=(0.05, 0.05), textAnchor=(0, 0),
                       content=data.get("wholeName", data.get("name", "未知地点")), color=(255, 255, 255, 255))

    now_pos = await card.addText(uvSize=(1, 1), boxSize=(0.7, 0.1), xy=(0, 0), baseAnchor=(0.05, 0.3),
                                 textAnchor=(0, 0), content="现有确诊: %s(%s)" % (confirmNow, confirmNowAdd),
                                 color=(255, 255, 255, 255))
    await card.addText(uvSize=(1, 1), boxSize=(0.7, 0.1), xy=(0, 0), baseAnchor=(0.05, 0.4), textAnchor=(0, 0),
                       content="累计确诊: %s(%s)" % (confirmTotal, confirmTotalAdd), color=(255, 255, 255, 255))
    await card.addText(uvSize=(1, 1), boxSize=(0.7, 0.1), xy=(0, 0), baseAnchor=(0.05, 0.5), textAnchor=(0, 0),
                       content="累计治愈: %s(%s)" % (healTotal,
                                                 healTotalAdd), color=(255, 255, 255, 255))
    await card.addText(uvSize=(1, 1), boxSize=(0.7, 0.1), xy=(0, 0), baseAnchor=(0.05, 0.6), textAnchor=(0, 0),
                       content="累计死亡: %s(%s)" % (deadTotal, deadTotalAdd), color=(255, 255, 255, 255))
    await card.addText(uvSize=(1, 1), boxSize=(0.7, 0.1), xy=(0, 0), baseAnchor=(0.05, 0.7), textAnchor=(0, 0),
                       content="境外输入: %s(%s)" % (inputNow, inputAdd), color=(255, 255, 255, 255))

    await card.addText(uvSize=(1, 1), boxSize=(0.7, 0.05), xy=(0, 0), baseAnchor=(0.95, 0.95), textAnchor=(1, 1),
