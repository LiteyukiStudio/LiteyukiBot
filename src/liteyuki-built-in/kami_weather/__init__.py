...
realTimeWeather = on_keyword(keywords={"天气"},
                             ...
                             priority=11,
                             block=True)
bindCity = on_command(cmd="绑定天气城市", priority=10, block=True)
helpWeather = on_command(cmd="天气参数", priority=10, block=True)
setDescription = on_command(cmd="设置城市描述", permission=SUPERUSER | MASTER, priority=10,
                            block=True)
setAdvice = on_command(cmd="设置天气建议", permission=SUPERUSER | MASTER, priority=10, block=True)
...
async def setAdviceHandle(bot: Bot, event: GroupMessageEvent | PrivateMessageEvent):
    try:
        ...
        adviceDict = await ExtraData.get_resource_data(key="kami.weather.advice", default={})
        advice = Command.formatToString(*args[2:]).replace("%20", " ")
        adviceDict[args[1]] = advice
        await ExtraData.set_resource_data(key="kami.weather.advice", value=adviceDict)
