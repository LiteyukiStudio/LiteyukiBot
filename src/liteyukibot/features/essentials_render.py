...
from .commands_models import CommandRegistration
from .commands_parsing import ArgumentSpec, CommandParseError, OptionSpec

type Language = Literal["zh-CN", "en"]

_DEFAULT_MESSAGES: dict[Language, dict[str, str]] = {
    "en": {
...
def messages(language: Language, translator: Translator) -> _Messages:
    ...
    defaults = _DEFAULT_MESSAGES[language]
    return _Messages(
        **{
            field: translator.text_for(locale, f"essentials.{field}", defaults[field])
...
def render_status(snapshot: KernelStatusSnapshot, *, language: Language, translator: Translator) -> str:
    ...
    lines = [
        ...
        f"{text.features}:",
        *_render_states(snapshot.features, empty=text.empty),
    ]
    return "\n".join(lines)
