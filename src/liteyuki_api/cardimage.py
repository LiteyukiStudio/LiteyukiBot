...
class Cardimage:
    ...
    def __init__(self, baseImg: Image.Image):
        ...
        self.fp = os.path.join(Path.cache, fileName)
        self.baseImg = baseImg.convert("RGBA")
    ...
    def addText(self, uvSize: Tuple[float, float], boxSize: Tuple[float, float],
                ...
                textAnchor: Tuple[float, float], content: str, font: str = os.path.join(Path.res, "fonts/MiSans-Medium.ttf"),
    ...
    def addImage(self, uvSize: Tuple[float, float], boxSize: Tuple[float, float],
        ...
        actualPos = baseAnchorXY[0] - textAnchorXY[0], baseAnchorXY[1] - textAnchorXY[1]
        alpha = img.split()[3]
        self.baseImg.paste(img, box=actualPos, mask=img)
        return actualPos[0] / self.baseImg.size[0] * uvSize[0], actualPos[1] / self.baseImg.size[1] * uvSize[1], (
                actualPos[0] + actualSize[0]) / self.baseImg.size[0] * uvSize[0], (actualPos[1] + actualSize[1]) / \
               self.baseImg.size[1] * uvSize[1]
    ...
    def convert(self, typ):
        self.baseImg = self.baseImg.convert(typ)
...
class Cardimage2:
    ...
    def addText(self, uvSize: Tuple[float, float], boxSize: Tuple[float, float],
                xyOffset: Tuple[float, float],
                basePoint: Tuple[float, float],
                selfPoint: Tuple[float, float], text: str, font: str = os.path.join(Path.res, "fonts/MiSans-Medium.ttf"),
                color: Tuple[int, int, int, int] = (255, 255, 255, 255), sizePercent: float = 1, force_size=False):
        """
        :param force_size: 强制大小
        :param sizePercent: 大小百分比,默认1
        ...
        initWidth, initHeight = limitedSize
        imageFont = ImageFont.truetype(font, size=initHeight)
        actualSize = imageFont.getsize(text)
        while (actualSize[0] > limitedSize[0] or actualSize[1] > limitedSize[1]) and not force_size:
            initHeight -= 5
            imageFont = ImageFont.truetype(font, size=initHeight)
            actualSize = imageFont.getsize(text)
        ...
        imageFont = ImageFont.truetype(font, size=initHeight * sizePercent)
        # 基类锚点绝对位置int
        baseAnchorXY = int(self.baseImg.size[0] * (basePoint[0] + xyOffset[0] / uvSize[0])), int(
            self.baseImg.size[1] * (basePoint[1] + xyOffset[1] / uvSize[1]))

        # 子对象锚点偏移量float|int
        textAnchorXY = int(actualSize[0] * selfPoint[0]), int(actualSize[1] * selfPoint[1])

        actualPos = baseAnchorXY[0] - textAnchorXY[0], baseAnchorXY[1] - textAnchorXY[1]

        draw.text(xy=actualPos, text=text, fill=color, font=imageFont)
        return actualPos[0] / self.baseImg.size[0] * uvSize[0], actualPos[1] / self.baseImg.size[1] * uvSize[1], (actualPos[0] + actualSize[0]) / self.baseImg.size[0] * uvSize[0], (
                    actualPos[1] + actualSize[1]) / self.baseImg.size[1] * uvSize[1]

    def addImage(self, uvSize: Tuple[float, float], boxSize: Tuple[float, float],
                 xyOffset: Tuple[float, float],
                 basePoint: Tuple[float, float],
                 selfPoint: Tuple[float, float], img: Image.Image,
                 sizePercent: float = 1):
        """
        :param img: 图片对象
        :param sizePercent: 大小百分比,默认1
        :param basePoint: 基底锚点[0-1, 0-1]
        :param selfPoint: 子对象锚点[0-1, 0-1]
        :param xyOffset: UV偏移量, 基于uvSize
        :param uvSize: UV分数
        :param boxSize: 对象大小, 基于uvSize
        :return:
    ...
    def cut(self, rate: Tuple[float, float], baseAnchor, imgAnchor):
        pass
