...
def load_text_hash_map() -> dict:
    ...
    default_lang = asyncio.run(Data(Data.globals, 'liteyuki').get('language', 'en'))
    logger.info(asyncio.run(get_text_by_language("000001", default_lang)).format(LANG_NAME=f"{asyncio.run(get_text_by_language('name', default_lang))}({default_lang})"))
load_text_hash_map()
