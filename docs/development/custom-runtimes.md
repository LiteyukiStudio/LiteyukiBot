...
The complete protocol-v6 child is in
...
Each source and target must name a distinct, enabled configured runtime. A
route must set `policy = "required" | "best_effort"` and
`completion = "sync" | "async"`; implicit defaults are rejected. Required
failure affects the event terminal state, while best-effort failure is retained
only as diagnostics. Sync waits for required delivery completion; async returns
after admission. The v6 compatibility runtime retains its historical
...
- send exactly one `EventAccepted` for each `EventMessage.delivery_id`;
- send exactly one terminal `EventCompleted` after each accepted core-to-child
  delivery, with the same delivery ID;
...
Protocol v6 Event receipt requires `runtime.events.receive`. An ingress runtime
sends `EventIngress(source_event_id, payload)` to the kernel. The kernel sends
an `EventMessage` carrying `kernel_event_id`, source provenance, `delivery_id`,
target runtime ID, positive attempt, lease ID, monotonic deadline, and payload.
Child-originated Actions and agent-tool requests require
`runtime.actions.send` and the active delivery ID; their lease is validated by
the kernel before a side effect is admitted. Capability names and protocol
versions are negotiated exactly rather than inferred.

Protocol v6 retains the narrow, capability-gated kernel control direction. It
is not a generic RPC surface. The protocol is pre-stable and has no compatibility
shim for v1 through v5. Pin the LiteyukiBot version used to build and test an
external runtime.
