...
def verify_bundle(
    ...
    *,
    ...
    if manifest.get("schema_version") != 1 or manifest.get("baseline") != dict(BUNDLE_BASELINE):
        raise BundleError("manifest schema or frozen Alpha15 baseline is invalid")
    ...
    expected_components = {
        "kernel": "liteyukibot-v7-kernel",
        "root": "liteyukibot-v7",
        "cordis": "liteyukibot-v7-cordis",
        "adapter-onebot": "liteyukibot-v7-adapter-onebot",
    }
    component_ids = [cast(dict[str, object], item).get("id") for item in components]
    if tuple(component_ids) != tuple(expected_components):
    ...
    for item in components:
        ...
        if component.get("reserved") is not False:
            raise BundleError("manifest component reserved state is invalid")
        if (
            component.get("distribution") != expected_components[component_id]
            or component.get("version") != BUNDLE_VERSION
        ):
            raise BundleError("manifest component identity is invalid")
