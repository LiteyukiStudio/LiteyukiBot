...
def _build_dir() -> Path:
    return Path(os.environ.get("LITEYUKI_BUILD_DIR", ROOT / "dist" / "workspace")).resolve()


def _example_build_dir() -> Path:
    override = os.environ.get("LITEYUKI_BUILD_DIR")
    if override is not None:
        return (Path(override) / "examples").resolve()
    return (ROOT / "dist" / "examples").resolve()
