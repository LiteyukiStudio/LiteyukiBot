...
const navigation: { id: Workspace; label: string; icon: typeof Activity }[] = [
  { id: "overview", label: "Overview", icon: Activity }, { id: "events", label: "Events", icon: Radio },
...
export function App() {
  ...
  return <TooltipProvider><div className="grid min-h-screen bg-background lg:grid-cols-[236px_minmax(0,1fr)]">
    ...
    <main className="min-w-0 px-4 py-6 sm:px-7 sm:py-8 lg:px-10 lg:py-7"><div className="mx-auto max-w-[1120px]">
      <header className="mb-7 flex items-start justify-between gap-3"><div className="flex items-start gap-3"><Button className="lg:hidden" variant="outline" size="icon" onClick={() => setMenuOpen(true)} aria-label="Open navigation"><Menu /></Button><div><p className="mb-1 font-mono text-[11px] font-semibold tracking-[.08em] text-muted-foreground">LOCAL INSTANCE</p><h1 className="text-[26px] font-semibold leading-none tracking-normal">{pageTitle}</h1><p className="mt-2 text-xs text-muted-foreground">{workspace === "overview" ? "Local operation state" : "Current daemon state"}</p></div></div><Tooltip><TooltipTrigger asChild><Button className="mt-0.5 rounded-md" variant="outline" size="icon" onClick={() => void reload()} aria-label="Refresh"><RefreshCw size={16} /></Button></TooltipTrigger><TooltipContent>Refresh snapshot</TooltipContent></Tooltip></header>
      <section className="webui-workbench"><WorkspaceView workspace={workspace} dashboard={dashboard} openOperation={setOperation} /></section>
    </div></main>
