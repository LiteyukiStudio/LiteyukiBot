...
function Configuration({ dashboard }: { dashboard: Dashboard }) { const { t } = useLocale(); return <Card><CardHeader><CardTitle>{t("webui.configuration.title")}</CardTitle></CardHeader><CardContent className="grid divide-y"><Setting label={t("webui.configuration.instance")} value={dashboard.instance} /><Setting label={t("webui.configuration.kernel")} value={dashboard.version} /><Setting label={t("webui.configuration.transport")} value={t("webui.configuration.loopback")} /><Setting label={t("webui.configuration.owner")} value={t("webui.configuration.daemon")} /></CardContent></Card>; }
...
function State({ value }: { value: string }) { const { t } = useLocale(); const positive = ["ready", "running", "enabled", "healthy", "succeeded"].includes(value); const warning = ["attention", "recovering", "queued", "running"].includes(value); return <Badge variant={positive ? "secondary" : warning ? "outline" : "destructive"} className={cn("capitalize", positive && "border-emerald-200 bg-emerald-50 text-emerald-700")}>{t(`webui.state.${value}`)}</Badge>; }
...
function Unavailable({ error, retry }: { error: string; retry: () => Promise<void> }) { const { t } = useLocale(); return <main className="grid min-h-screen place-items-center p-5"><Card className="w-full max-w-md"><CardHeader><CircleAlert className="mb-2 text-destructive" /><CardTitle>{t("webui.error.unavailable")}</CardTitle><CardDescription>{t("webui.error.unavailable_detail")}</CardDescription></CardHeader><CardContent className="flex items-center justify-between gap-3"><code className="text-xs text-muted-foreground">{error}</code><Button onClick={() => void retry()}><RefreshCw size={15} />{t("webui.action.retry")}</Button></CardContent></Card></main>; }
